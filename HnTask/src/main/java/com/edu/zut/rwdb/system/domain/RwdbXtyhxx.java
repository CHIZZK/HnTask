package com.edu.zut.rwdb.system.domain;

public class RwdbXtyhxx {

  private String gid;
  private String dwdm;
  private String yhdm;
  private String yhmc;
  private String xsmc;
  private String salt;
  private String javamm;
  private java.sql.Date dlsj;
  private java.sql.Date xxsj;
  private String stoped;
  private String sfzh;
  private String movetel;
  private String ipaddr;
  private String usertype;
  private String email;
  private String sex;


  public String getGid() {
    return gid;
  }

  public void setGid(String gid) {
    this.gid = gid;
  }


  public String getDwdm() {
    return dwdm;
  }

  public void setDwdm(String dwdm) {
    this.dwdm = dwdm;
  }


  public String getYhdm() {
    return yhdm;
  }

  public void setYhdm(String yhdm) {
    this.yhdm = yhdm;
  }


  public String getYhmc() {
    return yhmc;
  }

  public void setYhmc(String yhmc) {
    this.yhmc = yhmc;
  }


  public String getXsmc() {
    return xsmc;
  }

  public void setXsmc(String xsmc) {
    this.xsmc = xsmc;
  }


  public String getSalt() {
    return salt;
  }

  public void setSalt(String salt) {
    this.salt = salt;
  }


  public String getJavamm() {
    return javamm;
  }

  public void setJavamm(String javamm) {
    this.javamm = javamm;
  }


  public java.sql.Date getDlsj() {
    return dlsj;
  }

  public void setDlsj(java.sql.Date dlsj) {
    this.dlsj = dlsj;
  }


  public java.sql.Date getXxsj() {
    return xxsj;
  }

  public void setXxsj(java.sql.Date xxsj) {
    this.xxsj = xxsj;
  }


  public String getStoped() {
    return stoped;
  }

  public void setStoped(String stoped) {
    this.stoped = stoped;
  }


  public String getSfzh() {
    return sfzh;
  }

  public void setSfzh(String sfzh) {
    this.sfzh = sfzh;
  }


  public String getMovetel() {
    return movetel;
  }

  public void setMovetel(String movetel) {
    this.movetel = movetel;
  }


  public String getIpaddr() {
    return ipaddr;
  }

  public void setIpaddr(String ipaddr) {
    this.ipaddr = ipaddr;
  }


  public String getUsertype() {
    return usertype;
  }

  public void setUsertype(String usertype) {
    this.usertype = usertype;
  }


  public String getEmail() {
    return email;
  }

  public void setEmail(String email) {
    this.email = email;
  }


  public String getSex() {
    return sex;
  }

  public void setSex(String sex) {
    this.sex = sex;
  }

}
